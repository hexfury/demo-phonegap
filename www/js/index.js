...
var app = {
    ...
    sendEvent: function(type) {
        ...
        var authData = "grant_type=client_credentials";
        var auth = "https://auth.cxengage.net/1.0/token";
        $.ajaxSetup({headers: {'Authorization': "Basic " + btoa(cid + ':' + cs)}});
        var bearer = $.ajax({ url: auth, data: authData, type: "POST", done: function(result){
        ...
        var dest = "https://events.cxengage.net/1.0/tenants/qa/event"
        $.ajaxSetup({headers: {'Authorization': "Bearer " + bearer.val()}});
